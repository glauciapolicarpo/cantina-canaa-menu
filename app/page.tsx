...
function crc16(value: string) { let crc = 0xffff; for (let i = 0; i < value.length; i++) { crc ^= value.charCodeAt(i) << 8; for (let j = 0; j < 8; j++) crc = (crc & 0x8000) ? (crc << 1) ^ 0x1021 : crc << 1; crc &= 0xffff; } return crc.toString(16).toUpperCase().padStart(4, "0"); }
...
<h1>Cantina Canaã do Brasil</h1>
</div>
</header>
<section className="payment-screen">
<div className="payment-card form-card">
<div>
<p className="eyebrow accent">Acesso administrativo</p>
<h2>Entrar no<br/>
<em>painel.</em>
</h2>
<p>Use o e-mail autorizado para gerenciar o estoque.</p>
</div>
<form onSubmit={adminLogin}>
<label>E-mail<input type="email" value={adminEmail} onChange={e => setAdminEmail(e.target.value)} required />
</label>
<label>Senha<input type="password" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} required />
</label>{adminError && <p className="form-error">{adminError}</p>}<button className="primary-button" type="submit">Acessar painel <span>→</span>
</button>
</form>
</div>
</section>
...
<button className="help" onClick={() => { setAdminAuthenticated(false); setStep("menu"); }}>Sair</button>
</header>
<section className="payment-screen">
<div className="admin-heading">
<div>
<p className="eyebrow accent">Painel de estoque</p>
<h2>Gerencie seus<br/>
<em>itens.</em>
</h2>
</div>
<button className="back" onClick={() => setStep("menu")}>← Ver pedido</button>
</div>
<div className="stock-grid">{products.map(p => <article className="stock-card" key={p.id}>
<div className="food-icon">{p.emoji}</div>
<h3>{p.name}</h3>
<p>Disponível agora</p>
<strong>{inventory[p.id] ?? 0} unidades</strong>
<div className="stock-actions">
<button onClick={() => adjustStock(p.id, -1)}>-</button>
<button onClick={() => adjustStock(p.id, 1)}>+</button>
</div>
</article>)}</div>
<p className="form-note">Estoque salvo nesta sessão de protótipo.</p>
</section>
</main>;
  if (step === "admin") return <main className="app-shell">
<header className="topbar">
<div className="brand-mark">C</div>
<div>
<p className="eyebrow">Administração</p>
...
<section className="payment-screen">
<button className="back" onClick={() => setStep("menu")}>← Voltar ao pedido</button>
<div className="payment-card admin-card">
<div>
<p className="eyebrow accent">Agenda de pedidos</p>
<h2>Pedidos<br/>
<em>recebidos.</em>
</h2>
<p>Esta área lê os pedidos persistidos no backend/D1 quando a base estiver configurada.</p>
</div>
<div className="admin-list">
<p className="empty">Nenhum pedido carregado nesta sessão.</p>
<button className="primary-button" onClick={() => location.reload()}>Atualizar pedidos</button>
</div>
</div>
</section>
</main>;
...
<div className="brand-mark">C</div>
<div>
<p className="eyebrow">Faça seu pedido</p>
<h1>Cantina Canaã do Brasil</h1>
</div>
<button className="help" onClick={() => setStep("admin")} aria-label="Abrir administração">⌘</button>
</header>{step === "menu" && <>
...
<div className="hero-art">✦<span>feito<br/>com carinho</span>
</div>
</section>
...
<div>
<p className="eyebrow">Opções disponíveis</p>
<h3>Escolha seus itens</h3>
</div>
<span className="open-tag">● Aberto agora</span>
</div>
<div className="menu-list">{products.filter(p => (inventory[p.id] ?? 0) > 0).map(p => <article className="menu-item" key={p.id}>
<div className="food-icon">{p.emoji}</div>
<div className="item-info">
<span className="item-category">Por unidade</span>
<h4>{p.name} {p.id === "refri" && <small className="discount">preço menor</small>}</h4>
<p>{p.id === "refri" ? "Lata 350 ml" : "R$ 8,00 por unidade"}</p>
<strong>{money(p.price)}</strong>
</div>
<div className="qty product-qty">
<button onClick={() => change(p.id, -1)}>-</button>
<b>{quantities[p.id] ?? 0}</b>
<button onClick={() => change(p.id, 1)}>+</button>
</div>
</article>)}</div>
</section>
...
<div className="cart-items">{selected.length ? selected.map(i => <div className="cart-line" key={i.id}>
<div>
<h4>{i.quantity}x {i.name}</h4>
<span>{money(i.price)} cada</span>
</div>
<strong>{money(i.price * i.quantity)}</strong>
</div>) : <p className="empty">Escolha os itens ao lado.</p>}</div>
<div className="cart-total">
<span>Total final</span>
<strong>{money(total)}</strong>
</div>
<button className="primary-button" disabled={!count} onClick={() => setStep("receipt")}>Continuar <span>→</span>
</button>
</aside>
</div>
</>}{step === "receipt" && <section className="payment-screen">
<button className="back" onClick={() => setStep("menu")}>← Voltar para a sacola</button>
<div className="payment-card form-card">
<div>
<p className="eyebrow accent">Antes do pagamento</p>
<h2>Escreva<br/>
<em>seu nome</em>
<small className="name-hint">para validar na sua comanda</small>
</h2>
<p>Se o pagamento for via Pix, você deverá anexar o comprovante diretamente no WhatsApp.</p>
</div>
<form onSubmit={e => { e.preventDefault(); createOrder(); }}>
<label>Escreva seu nome<input value={name} onChange={e => setName(e.target.value)} placeholder="Ex.: Maria Silva" required />
</label>
<fieldset className="payment-options">
<legend>Escolha a forma de pagamento</legend>
<label className="payment-option">
<input type="radio" name="paymentMethod" value="cash" checked={paymentMethod === "cash"} onChange={() => setPaymentMethod("cash")} /> À vista</label>
<label className="payment-option">
...
</label>}<p className="form-note">
</p>{error && <p className="form-error">{error}</p>}<button className="primary-button" type="submit" disabled={isSubmitting}>{submitText} <span className={isSubmitting ? "loading-spinner" : ""} aria-hidden={isSubmitting}>{submitIcon}</span>
</button>
</form>
</div>
